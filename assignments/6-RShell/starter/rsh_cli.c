...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1) 
    {
        ...
        ssize_t sent = send(cli_socket, cmd_buff, len, 0);
        if (sent < 0) {
        ...
        int isEOF = 0;
          while(!isEOF){
            ...
            // check last byte for RDSH_EOF_CHAR:
            isEOF = (rsp_buff[recv_bytes-1] == RDSH_EOF_CHAR) ? 1 : 0;
            if(isEOF){
               rsp_buff[recv_bytes-1] = '\0'; // remove the EOF char
            }
